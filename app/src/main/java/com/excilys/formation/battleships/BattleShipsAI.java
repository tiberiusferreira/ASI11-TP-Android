...
public class BattleShipsAI implements Serializable {
     ...
     */
    BattleShipsAI(IBoard myBoard, IBoard opponentBoard) {
        this.board = myBoard;
     ...
     */
    void putShips(List<AbstractShip> ships) {
        // Get a list of possible orientation values in order to later choose a random one
        final List<AbstractShip.Orientation> orientationList = Collections.unmodifiableList(Arrays.asList(AbstractShip.Orientation.values()));
        Random rand = new Random();
        AbstractShip.Orientation orientation;

        for (int i=0; i<ships.size(); ) {
                // Get a random X and Y value inside the board to put the ship
                int x = rand.nextInt((board.getSize() - 1 - 0) + 1) + 0;
                int y = rand.nextInt((board.getSize() - 1 - 0) + 1) + 0;
                // Choose a random orientation
                orientation = orientationList.get(rand.nextInt(orientationList.size()));
                // Set ship orienation
                ships.get(i).setOrientation(orientation);
                // Try to put it. If has been put change to next ship
                if(board.putShip(ships.get(i), x, y) == 1){
                    i++;
                }
        }
    }
     ...
     * Private
     */
    // I don't use this, I prefer to use the method inside the Board itself because it already checks
    // if the ship can be put
